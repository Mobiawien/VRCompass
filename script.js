...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tabella3Body = document
    .getElementById("tabella3")
    .querySelector("tbody");
  ...
  const outputSpesaEffettiva = document.getElementById(
    "output-spesa-effettiva"
  );
  ...
  const tabellaClassificaVsr = document.getElementById(
    "tabella-classifica-vsr"
  );
  ...
  const listaSuggerimentiSlot = document.getElementById(
    "lista-suggerimenti-slot"
  );
  ...
  const vsrChangeNotification = document.getElementById(
    "vsr-change-notification"
  );
  // Aggiungiamo i riferimenti specifici per messaggio e pulsante di chiusura
  const vsrNotificationMessage = document.getElementById(
    "vsr-notification-message"
  );
  const vsrNotificationCloseBtn = document.getElementById(
    "vsr-notification-close-btn"
  );
  ...
  function aggiornaTestiPulsantiFormGara() {
    const submitButton = formAggiungiGara.querySelector(
      'button[type="submit"]'
    );
    if (submitButton)
  ...
  function aggiornaInfoClassificaView() {
    ...
    const classificaVsrRaw =
      localStorage.getItem("classificaVsrAttuale") || "0";
    ...
    if (classificaVsrAttualeDisplay)
      classificaVsrAttualeDisplay.textContent = formatNumber(
        classificaVsrNum,
        0
      );
  ...
  function aggiornaTabella2() {
    ...
    if (outputSpesa)
      outputSpesa.textContent = formatNumber(spesaAttrezzature, 0);
    if (outputSpesaEffettiva)
  ...
  function ricalcolaRisultatiTabella3() {
    if (
      ...
      if (cellaCreditiTab3Ref)
        cellaCreditiTab3Ref.textContent = getTranslation("TEXT_NA_DETAILED");
      if (cellaNettoTab3Ref)
        cellaNettoTab3Ref.textContent = getTranslation("TEXT_NA_DETAILED");
      if (cellaPuntiTab3Ref)
        cellaPuntiTab3Ref.textContent = getTranslation("TEXT_NA_DETAILED");
      return;
    }
  ...
  function rimuoviEvidenziazioneTutteLeRighe() {
    ...
    righeEvidenziate.forEach((riga) =>
      riga.classList.remove("riga-in-modifica")
    );
  }
  ...
  function getContributingGareIds() {
    ...
    const gareContributive =
      selezionaGareContributivePerClassifica(gareSalvate);
  ...
  function popolaERendiVisibileRiepilogoEventi() {
    ...
    let htmlContent = `<h4>${getTranslation(
      "SUMMARY_RECENT_EVENTS_TITLE"
    )}</h4>`;
    ...
    reader.onload = function (e) {
      try {
        ...
        if (Array.isArray(datiImportati.gareSalvate)) {
          ...
          alert(getTranslation("ALERT_INVALID_SUGGESTED_RACES_JSON_ARRAY"));
      } catch (error) {
        console.error(
  ...
  function getGareContributiveConDettagli() {
    return selezionaGareContributivePerClassifica(
      JSON.parse(localStorage.getItem("gareSalvate")) || [],
      null
    );
  }
  ...
  function aggiornaPanoramicaSlotVSR() {
    ...
    function popolaCategoriaSlot(
      ...
      let chiaveMappaPerValoreNumerico = Object.keys(livelliVsrStoricoMap).find(
      ...
      if (gare.length > 0) {
        ...
        gare.forEach((g) => {
          ...
          let params = {
            ...
            directImpactPoints: formatNumber(
              Math.abs(impattoDirettoGaraVal),
              0
            ),
          ...
          if (
            ...
            if (primaGaraBeneficiariaNonSimulata) {
              ...
              params.beneficiaryRaceName =
                primaGaraBeneficiariaNonSimulata.nome;
          ...
          if (g.isUrgente) {
            params.remainingDays = calcolaGiorniTraDate(
              new Date(),
              g.dataEvento
            );
            params.daysText =
            ...
            if (garaEsceDalRanking) {
              ...
              testoAvvisoKey =
                "STRATEGY_DEADLINE_ITEM_URGENT_REBALANCE_POSITIVE";
            ...
            if (garaEsceDalRanking) {
              ...
              testoAvvisoKey =
                "STRATEGY_DEADLINE_ITEM_NORMAL_REBALANCE_POSITIVE";
            } else if (laGaraStessaEntraDaNonContribuente) {
              testoAvvisoKey = "STRATEGY_DEADLINE_ITEM_NORMAL_ENTERS_ON_EVENT";
            } else if (differenzaImpattoTrascurabile) {
  ...
  function aggiornaValutazioneStrategicaSlot() {
    ...
    try {
      ...
      ordineTipiPerStrategia.forEach((tipoGara) => {
        ...
        let chiaveMappaPerValoreNumerico = Object.keys(
        ...
        if (dimezzamentiSonoStatiCompensati && slotAttualmenteVuoti100 <= 0) {
          ...
          let puntiEsempio = calcolaPuntiPerClassifica(
            livelloValoreNumerico,
            50
          );
              ...
              tooltip: {
                callbacks: {
                  label: function (tooltipItem) {
                    ...
                    let percentualePuntiVSRPerCategoria =
                      potenzialeMaxCategoria > 0
                        ? (puntiAttualiDellaCategoria /
                            potenzialeMaxCategoria) *
                          100
                        : 0;
                    tooltipLines.push(
                      `${getTranslation(
                    ...
                    const numGare100AttualiTooltip =
                      gare100AttualiTooltip.length;
  ...
  function calcolaPunteggioSaluteCategoria(
    gareCat,
    maxSlotPerFascia,
    valoreMaxPuntiGara
  ) {
    ...
    if (gareCat.length > 0 && valoreMaxPuntiGara > 0) {
      let sommaQualitaPercentuale = gareCat.reduce((sum, g) => {
        const potenzialeMaxSlotSpecifico =
          valoreMaxPuntiGara * g.fattoreDecadimento;
        return (
          sum +
          (potenzialeMaxSlotSpecifico > 0
            ? g.puntiEffettivi / potenzialeMaxSlotSpecifico
